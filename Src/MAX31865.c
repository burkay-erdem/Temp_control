...
void spi_write(uint8_t *data, uint8_t len)
{
    for (uint8_t x = 0; x < len; x++)
    {
        ...
        {
            HAL_GPIO_WritePin(GPIOA, GPIO_PIN_7, (data[x] & (1 << i)));
            DELAY(1);
...
void spi_read(uint8_t *buffer, uint8_t len)
{
    for (uint8_t x = 0; x < len; x++)
    {
        ...
        for (int8_t i = 7; i >= 0; i--)
        {
            ...
            HAL_GPIO_WritePin(GPIOA, GPIO_PIN_5, 1);
            DELAY(1);
            buffer[x] |= HAL_GPIO_ReadPin(GPIOA,GPIO_PIN_6);
            DELAY(1);
            HAL_GPIO_WritePin(GPIOA,GPIO_PIN_5, 0);
        }
    }
...
void MAX31865_read(uint8_t addr, uint8_t *buffer, uint8_t len)
{
    ...
    HAL_GPIO_WritePin(GPIOE, GPIO_PIN_7, 0);          // Enable CE

    spi_write(&addr, 1);                                        // Write addr
    spi_read(buffer, len);                                      // Read data

    HAL_GPIO_WritePin(GPIOE, GPIO_PIN_7, 1);          // Disable CE
}
 ...
 */
void MAX31865_init(uint8_t wires)
{
